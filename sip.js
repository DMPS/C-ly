...
(function(sip){
    ...
    sip.map = function(lambda, list) {
        //console.log("sip.map() list:" + list)
        var result = [];
        for (var i=0; i<list.length; ++i) {
            //console.log("sip.map() i:" + i + ", list[i] :" + list[i])
            result.push(lambda(list[i]));
        }
        //console.log("sip.map() result:" + result)
        return result;
    };
...
(function(sip){
    ...
    sip._exception = {'call-id':'Call-ID','cseq':'CSeq','www-authenticate':'WWW-Authenticate','x-alu-authorization':'X-ALU-Authorization'};
    ...
    // ALU begin
    sip._getValues = function(value) {
        var list =[];
        var startStr = false;
        var startPos = 0;
        
        if (value.indexOf(',') == -1 && value.toLowerCase().indexOf("%2c") == -1) {
            list.push(value);
        } else {  
            for (var i=0,j=value.length; i<j; i++){
                if (value[i] == '%') {
                    var str = value.toLowerCase().substr(i,3);
                    if ( str == "%22" || str == "%27") {
                        startStr = ! startStr;
                    } else if (str == "%2c") {
                        if (startStr == false) {
                            list.push(value.substring(startPos, i));
                            startPos = i+1;
                        }
                    }
    ...
    Header.createHeaders = function(value) {
        ...
        if (sip._comma.indexOf(name.toLowerCase()) >= 0) {
            ...
            //console.log("createHeaders() begin");
            // ALU begin
            //headers = sip.map(function(item) { return new sip.Header(item, name); }, value.split(','));
            headers = sip.map(function(item) { return new sip.Header(item, name); }, sip._getValues(value));
            // ALU end
            //console.log("createHeaders() end");
            //console.log("createHeaders() headers:" + headers);
        }
    ...
    Message.prototype._parse = function(value) {
        ...
        // ALU begin
        // support header'value set in several lines like WWW-Authenticate header
        var concatenation = false;
        for (var i=0; i<parts.length; ++i) {
            if (parts[i] != undefined) {
                if (parts[i].search(',\r') != -1) {
                    ...
                    parts[j] = parts[j].concat(parts[i+1]); // concat with the next one
                } else {
            ...
            // if (parts[i] != undefined && parts[i].search(',\r') != -1) {
                // if (parts[i-1] != undefined) {
                    // parts[j] = parts[j].concat(parts[i]); // concat with the first one
                    // parts[i] = '';     
                    // concatenation = true;                
                // } else {
                    // // the first one
            ...
            // } else {
                // if (concatenation == true) {
                    // parts[j] = parts[j].concat(parts[i]); // concat the last part with the first one
                // }
                // concatenation = false;
            // }
        }
        ...
        this.close = function() {
            ...
            if (this.stack) {
                if (this.stack.transactions[this.id] !== undefined) {
                    //console.log("close() close transaction : " + this.id)
    ...
    Transaction.createBranch = function(request, server) {
        ...
        var data = to.toString().toLowerCase() + "|" + from.toString().toLowerCase() + "|" + callid.toString() + "|" + cseq.toString()  + "|" + localtag  + "|" + ((remotetag != null)?remotetag:'');
        // ALU end
    ...
    Transaction.createServer = function(stack, app, request, transport, tag, start) {
        ...
        //console.log("Transaction.createServer() add transaction: " + t.id)
    ...
    Transaction.createClient = function(stack, app, request, transport, remote) {
        ...
        t.id = Transaction.createId(t.branch, request.method);
        //console.log("Transaction.createClient() add transaction: " + t.id)
        stack.transactions[t.id] = t;
        t.start();
        return t;
    };
    ...
    function ClientTransaction() {
        ...
        //console.log("ClientTransaction.receivedResponse");
            if (response.is1xx()) {
    ...
    UserAgent.prototype.createRequest = function(method, content, contentType) {
        ...
        if (method == 'REGISTER') 
            this.callId = this.stack.getNewCallId();        
        var CallId = new sip.Header(this.callId, 'Call-ID');
        // ALU end
        var MaxForwards = new sip.Header("" + this.maxForwards, 'Max-Forwards');
        var Via = this.stack.createVia(this.secure);
        // ALU begin
        Via.branch = sip.Transaction.createBranch([To.value, From.value, CallId.value, CSeq.number, this.localTag, this.remoteTag], false);
        ...
        if (method != 'MESSAGE') {
            ...
            var headers = [To, From, CSeq, CallId, MaxForwards, Via];
        }
        // ALU end
    ...
    UserAgent.prototype.canCreateDialog = function(request, response) {
        ...
        return (response.is2xx() && (request.method == "INVITE" || request.method == "SUBSCRIBE")) || (response.is1xx() && response.getItem('To')['tag'] !== undefined);
        // ALU end  
    };
    ...
    UserAgent.prototype.receivedResponse = function(transaction, response) {
        if (transaction && transaction !== this.transaction) {
            // ALU begin
            if (transaction && transaction !== this.cancelTransaction) {
            // ALU end
        ...
        if (response.is1xx()) {
            ...
            if (this.canCreateDialog(this.request, response)) {
                ...
                if (dialog.autoack && this.request.method == 'INVITE' && response.getItem('CSeq')['method'] == 'INVITE') {
                // ALU end
    ...
    UserAgent.prototype.receivedRequest = function(transaction, request) {
        ...
        if (request.getItem("To")["tag"] === undefined && request.method != 'CANCEL') {
        // ALU end
        ...
        // ALU begin
        //if (transaction)
        if (transaction && request.method != 'CANCEL')
        // ALU end
    ...
    UserAgent.prototype.sendResponse = function(response, responsetext, content, contentType, createDialog) {
        ...
        else if (createDialog && this.canCreateDialog(this.request, response)) {
        // ALU end
    ...
    UserAgent.prototype.createResponse = function(response, responsetext, content, contentType) {
        ...
        //if (!response.is1xx() && response.getItem("To")["tag"] === undefined)
        // ALU end
    ...
    UserAgent.prototype.sendCancel = function() {
        ...
        this.cancelRequest.setItem("Max-Forwards", new sip.Header("" + this.maxForwards, "Max-Forwards"));
    ...
    UserAgent.prototype.authenticate = function(response, transaction) {
        ...
        for (var i=0; i<auths.length; ++i) {
            var b = auths[i];
            console.log("handling b=" + b);
            if (   ( a.realm == b.realm )
                && (   (   ( a.name == 'WWW-Authenticate' )
                        && ( b.name == 'Authorization' ) )
                    || (   ( a.name == 'Proxy-Authenticate' )
                        && ( b.name == 'Proxy-Authorization') ) ) )
            {
                present = true;
                break;
            }
    ...
    Dialog.createServer = function(stack, request, response, transaction) {
        ...
        d.stack.log('request contact ' + request.Contact);
        // ALU end
    ...
    Dialog.prototype.sendCancel = function() {
        if (this.clients.length == 0) {
            ...
            d.stack.log('No client transaction to send cancel');
            // ALU end
            return;
        }
    ...
    Dialog.prototype.receivedRequest = function(transaction, request) {
        if (this.remoteSeq != 0 && request.getItem("CSeq").number < this.remoteSeq) {
            ...
            d.stack.log('Dialog.receivedRequest() CSeq is old ' + request.getItem("CSeq").number + ' < ' + this.remoteSeq);
            // ALU end
    ...
    Dialog.prototype.receivedResponse = function(transaction, response) {
        ...
        // ALU begin
        if (!response.is1xx() || (response.is1xx() && response.getItem('To')['tag'] !== undefined))
        //if (!response.is1xx())
        // ALU end
    ...
    Stack.prototype.close = function() {
        ...
        for (var t in this.transactions) {
                    //console.log("Stack.prototype.close() close transaction : " + t)
    ...
    // ALU begin
        ...
        return new sip.Header('SIP/2.0/' + this.transport.type.toUpperCase() + ' ' + this.transport.hostport, 'Via');
    // ALU end
    };
    ...
    Stack.prototype.received = function(data, src) {
        ...
        try {
            ...
            if (m.method) {
                ...
                this.log("received response=" + m.response + " " + m.responsetext);
            ...
            this.log('Error in received message: ' + e);
                this.log(e.stack);
...
//        }
        ...
        if (r.method == 'ACK') {
            d = this.findDialog(r);
            if (d != null) {
                if (d.transaction instanceof InviteServerTransaction && d.transaction != null) {
                    lastResponse = d.transaction.lastResponse;
                    if (lastResponse.isfinal()) {
                        if (lastResponse.is2xx()) {
                            t = null;
                        } else {
                            this.log("Found existing transaction for ACK on non-2xx final response");
                            t = d.transaction;
                        }
                    }
                } else {
        ...
        if (!t) {
            ...
            // ALU end
                ...
                if (r.method == 'ACK' && t && this.transactions[t.id] !== undefined) {
                    //console.log("Stack.prototype._receivedRequest() delete transaction: " + t.id)
                    delete this.transactions[t.id];
                }
            }
            else if (app && r.method == 'ACK') {
                app.receivedRequest(null, r);
            }
            else if (r.method != 'ACK') {
            ...
            this.log("found existing transaction");
    ...
    Stack.prototype._receivedResponse = function(r, uri) {
        ...
        if (!t) {
            if (method == 'INVITE' && r.is2xx()) {
                ...
                this.log('transaction id ' + sip.Transaction.createId(branch, method) + ' not found');
...
(function(sip){
    ...
    sip.createAuthorization = function(challenge, username, password, uri, method, entityBody, context) {
        ...
        if (authMethod.toLowerCase() == 'basic') {
            ...
            //return authMethod + ' ' + sip.map(function(y) { return y + '=' + (y == 'algorithm'  ? cr[y] : sip._quote(cr[y]));}, items).join(',');
        }
        else {
            throw new String('Invalid auth method -- ' + authMethod);
        }
