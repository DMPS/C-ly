		...
		if (type && this.body) {
			...
			if (this.body instanceof String || typeof this.body === 'string') {
				...
				if(this.body instanceof Uint8Array) {
					console.log("Sending ArrayBuffer/Uint8Array") ;

					if(this.body instanceof File) {
						console.log("this.body is File") ;
					} else if(this.body instanceof Blob) {
						console.log("this.body is Blob") ;
					} else if(this.body instanceof ArrayBuffer) {
						console.log("this.body is ArrayBuffer") ;
					} else {
						console.log("this.body is other") ;
					}

					totallen = msg.length+this.body.length+lineEnd.length+end.length ;
					console.log("Using ArrayBuffer instead of Blob. length: " + totallen + ",this.body.length:" + this.body.length ) 
						ab = new ArrayBuffer(totallen) ;
					//ab = new ArrayBuffer(msg.length+lineEnd.length+end.length) ;
					uia = new Uint8Array(ab);
					for (i = 0, j=0; i < msg.length; i++,j++) {
						uia[i] = msg.charCodeAt(j);
					}

					for (j=0; j < this.body.length; j++,i++) {
						//uia[i] = this.body.charCodeAt(j);
						uia[i] = this.body[j] ;
					}
					for (j=0; j < lineEnd.length; j++,i++) {
						uia[i] = lineEnd.charCodeAt(j);
					}
					for (j=0; j < end.length; j++,i++) {
						uia[i] = end.charCodeAt(j);
					}
					msg = ab ;
					console.log("Sending chunk of length:" + uia.length ) ;
					...
					readBinaryStringFromArrayBuffer(uia, 
							function(str) {console.log("chunk contents  (length: " + str.length + "): " + str) ;} , 
							function(error) {console.log("chunk not sent. error" + error) ;}) ;
				} else {
