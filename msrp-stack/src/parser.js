...
var CrocMSRP = (function(CrocMSRP) {
	...
	CrocMSRP.parseMessage = function(data) {
		...
		if (data instanceof ArrayBuffer) {
			...
			// Raju:TODO: We can't get ArrayBuffer as the data has to be converted to string so that MSRP msg boundaries have to be found.
		...
		if (msg.substr(startIndex, lineEnd.length) === lineEnd) {
			...
			if (data instanceof ArrayBuffer) {
				...
				//Raju
				console.log("Incoming chunk: ArrayBuffer. startIndex "+ startIndex + ", endIndex " + endIndex) ;
				msgObj.body = data.slice(startIndex, endIndex);
			} else {
				//Raju
				// Assume we're only dealing with text
				console.log("parseMessage(): Incoming chunk: NOT ArrayBuffer. startIndex "+ startIndex + ", endIndex " + endIndex) ;
				msgObj.body = msg.substring(startIndex, endIndex);
				console.log("parseMessage msgObj.body:" + msgObj.body) ;
			}
	...
	//ALU start
	function parseContentType(headerArray, msgObj) {
		...
		if (headerArray[0]) {
			msgObj.contentType = chomp(headerArray[0]);
			if (msgObj.contentType.length < 1 ||
				msgObj.contentType.match(/message\/CPIM|text\/plain|application\/octet-stream/)) {
	...
	function parseContentTransferEncoding(headerArray, msgObj) {
		// We expect one or no header
		if (headerArray.length > 1) {
			console.log('Multiple Content-Transfer-Encoding headers');
			return false;
		}
		if (headerArray[0]) {
			msgObj.contentTransferEncoding = chomp(headerArray[0]);
			if (msgObj.contentTransferEncoding.length < 1) {
	...
	var headerParsers = {
		...
		'Content-Transfer-Encoding': parseContentTransferEncoding,
		//ALU end
