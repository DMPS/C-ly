...
var CrocMSRP = (function(CrocMSRP) {
	...
	CrocMSRP.ChunkSender = function(session, body, contentType, disposition, description) {
		...
		if (!body) {
			...
			console.log("ChunkSender Body is of type ArrayBuffer... keeping blob as ArrayBuffer ") ;
			// Stop Chrome complaining about ArrayBuffer in Blob constructor
			// Raju: keep it as ArrayBuffer or ArrayBuffer View and then
			// during getNextChunk just specify the offset or create Blob
			//this.blob = new Blob([new Uint8Array(body)]);
			this.arraybuffer= new Uint8Array(body) ; 
			this.contentType = contentType || 'application/octet-stream';
			//this.contentType = contentType || 'message/cpim';
			this.disposition = disposition;
		} else if (body instanceof ArrayBufferView) {
			console.log("ChunkSender Body is of type ArrayBufferView ") ;
			this.blob = new Blob([body]);
			this.contentType = contentType || 'application/octet-stream';
			this.disposition = disposition;
		} else {
		...
		if(this.arraybuffer) {
			this.size = this.arraybuffer.length
		} else {
			this.size = this.blob.size;
		}

		console.debug("Total size of the file to send: " + this.size) ;
	...
	CrocMSRP.ChunkSender.prototype.getNextChunk = function() {
		...
		if (this.aborted) {
			...
			if (this.size > 0) {
				...
				//Raju:Begin
				if(this.arraybuffer) {
					chunk.body = this.arraybuffer.subarray(this.sentBytes, end);
					console.debug("Slicing the ArrayBuffer for next chunk from " + this.sentBytes + "-" + end + ", chunk.body.length: "+ chunk.body.length) ;
				} else {
					chunk.body = this.blob.slice(this.sentBytes, end);
					console.debug("Slicing the blob for next chunk from " + this.sentBytes + "-" + end + "chunk.body.size: "+ chunk.body.size) ;
				}
				//Raju:End
			}
